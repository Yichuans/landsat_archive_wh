...
# -------------- NEED MODIFICATION ---------------
from pansharp import pansharpen, folder_has_pansharpen

WDPAID = 555577555

data_depo = r"E:\Yichuan\Landsat_archiving" + os.sep + WDPAID

def job(job_id):
    # the result should catch errors
    result = None
    result = pansharpen(job_id)

    if not result:
        print('Error: no result produced, zero is returned')
        return 0
...
def get_queue():
    ...
    for folder in os.listdir(data_depo):
        folder = data_depo + os.sep + folder
        if not folder_has_pansharpen(folder):
            q.put(folder)
